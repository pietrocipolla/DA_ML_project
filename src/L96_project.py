...
class L96TwoLevel(object):
    def __init__(self, K=36, J=10, h=1, F=10, c=10, b=10, dt=0.001,
        ...
        self.Y = np.random.rand(self.K * self.J) if Y_init is None else Y_init.copy()
    ...
    def iterate(self, time, noprogress=False):
        steps = int(time / self.dt)
        for n in tqdm(range(steps), disable=noprogress):
            self.step()
